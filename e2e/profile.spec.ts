...
test.describe("Profile page", () => {
  ...
  test("Should display user profile information", async ({ page }) => {
    ...
    await expect(
      page.locator(".v-breadcrumbs-item").getByText("Profile"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("main").getByText(USER_EMAIL).first(),
    ).toBeVisible();

    // Verify quiz settings sections
    await expect(
      page.getByRole("heading", { name: "Questions in quiz:" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Questions in quiz (repeat mode):" }),
    ).toBeVisible();

    // Verify avatar section
    await expect(
      page.getByRole("heading", { name: "Upload photo" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("textbox", { name: "Repeat password Repeat password" }),
    ).toBeVisible();
  });
  ...
  test("Should edit and save questions in quiz setting", async ({ page }) => {
    ...
    const quizSection = page
      .getByRole("heading", { name: "Questions in quiz:" })
      .locator("..");
  ...
  test("Should edit and save questions in quiz (repeat mode) setting", async ({
    page,
  }) => {
    // Navigate to profile
    await page.getByRole("button", { name: "Avatar" }).click();
    ...
    const currentValue = page
      .getByRole("heading", { name: "Questions in quiz (repeat mode):" })
      .locator("..");
